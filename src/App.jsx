...
{screen === "history" && (
  <div className="absolute inset-0 z-30 flex items-center justify-center px-6">
    <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto animate-[fadeIn_0.8s_ease-out]">
      <div className="rounded-[2rem] border border-white/60 bg-white/90 p-8 shadow-2xl backdrop-blur-md">
        ...
        <div className="mt-5 space-y-3 max-h-80 overflow-y-auto">
          {JSON.parse(localStorage.getItem('bloom_history') || '[]').slice().reverse().map((item, i) => (
            <div key={i} className="rounded-2xl bg-[#f5fbf6] p-4 border border-[#dff0e6]">
              <div className="flex items-center justify-between">
                <span className="font-medium text-[#234b36]">
                  {item.type === 'breath' && '🌬️ Took a breath'}
                  {item.type === 'notice' && '👀 Noticed things'}
                  {item.type === 'write' && '✍️ Wrote something'}
                </span>
              ...
              {item.content && (
                <p className="mt-1 text-sm text-[#527060] italic">"{item.content}"</p>
              )}
              {item.feeling && (
                <p className="mt-1 text-xs text-[#78ad70]">Feeling: {item.feeling}</p>
              )}
            </div>
          ))}
          {JSON.parse(localStorage.getItem('bloom_history') || '[]').length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-[#527060]">No history yet.</p>
              <p className="text-sm text-[#78ad70]">Your first moment will appear here.</p>
            </div>
          )}
        </div>
